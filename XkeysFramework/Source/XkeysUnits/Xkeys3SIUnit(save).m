...
//static const uint8_t XK_INPUT_REPORT_LENGTH = 36; //this is read in from device now
//static const uint8_t XK_OUTPUT_COMMAND_LENGTH = 35; //this is read in from device now
static const uint8_t XK_GENERATE_DATA_COMMAND = 177;
static const uint8_t XK_GENERATE_DATA_REPLY = 2; //this could be 3 if program switch is held down or in case of stick keys and matrix set in the on position
...
static const uint8_t XK_BUTTON_COUNT = 124;
...
- (instancetype)initWithHIDSystem:(id<XkeysHIDSystem>)hidSystem device:(IOHIDDeviceRef)hidDevice connection:(id<XkeysConnection>)connection {
    ...
    //code for selecting the proper endpoint
    ...
    if ( ! connection.receivesData ) {
        ...
        targetUsage = kHIDUsage_GD_Mouse; //currently all X-keys devices in a hardware mode pid have a mouse endpoint
    }
    ...
    if ( self.writeLength < 10) //device in hardware mode, hardcode a writelength
    {
        self.writeLength=35; //all X-keys have same writelength
    }
    ...
    //this is important can't remove or button callback won't work
...
- (void)setProductID:(NSInteger)productID { //not in use
    
    NSArray *validPIDs = @[ @1221, @1222, @1223, @1224 ];
    NSAssert( [validPIDs containsObject:@(productID)], @"Xkeys XK-3 Switch Interface PIDs must be 1221-1224 (not %ld)", productID);
...
- (void)initialUnitStateConfigured {
    ...
    //who cares about the stupid led state, take out for now
...
- (void)open {
    ...
    if ( self.connection.receivesData ) {
        ...
        [self sendGeneralDataReportRequest]; //this will generate an input report which will then show the gui
...
- (void)startListeningForInputReports {
    ...
    if (_reportBufferIn == nil)
        _reportBufferIn = (uint8_t *)malloc (self.readLength);
    
    [self.hidSystem device:self.hidDevice registerInputReport:_reportBufferIn length:sizeof(_reportBufferIn) callback:Xkeys3SIInputReportCallback context:context];
}
...
- (void)stopListeningForInputReports {
    ...
    [self.hidSystem device:self.hidDevice registerInputReport:_reportBufferIn length:sizeof(_reportBufferIn) callback:NULL context:NULL];
}
...
- (void)sendReportWithByte0:(uint8_t)byte0 byte1:(uint8_t)byte1 byte2:(uint8_t)byte2 {
    ...
    uint8_t _reportBufferOut[MIN(self.writeLength, 3)];
    bzero(_reportBufferOut, sizeof(_reportBufferOut));
    
    _reportBufferOut[0] = byte0;
    _reportBufferOut[1] = byte1;
    _reportBufferOut[2] = byte2;

    [self.connection sendReportBytes:_reportBufferOut ofLength:self.writeLength];
}
...
void Xkeys3SIInputReportCallback( void *context, IOReturn result, void *sender, IOHIDReportType type, uint32_t reportID, uint8_t *report, CFIndex reportLength) {
    ...
    NSString *joinString=@"";
    for (int i=0;i<unit.readLength;i++)
    {
   ...
   // [unit invokeOnAnyButtonCallbackWithButton:0]; //for raw data to show if send d6, e0 or other output reports that send back a response. Removed per chuck downs
    
    //this stuff was there from Ken's original, not my mods
    if ( replyDataType == XK_GENERATE_DATA_REPLY || replyDataType == 3) {
        ...
        if (_firsttime==0)
        {
            ...
            [unit initialUnitStateConfigured]; //must have this but this is not the place
        }
    }
